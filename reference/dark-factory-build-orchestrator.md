...
6. [`work-autonomously`](../skills/work-autonomously/SKILL.md) — gates the action: act in dev/non-prod, HARD-STOP at real-world boundaries (below).
...
Per [`work-autonomously`](../skills/work-autonomously/SKILL.md): **default is action in dev/test/local/non-prod; resolve ambiguity via the toolchain (memory → codebase → internet → docs) before asking.**
