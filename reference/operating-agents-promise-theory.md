...
> **Trust attaches to a pinned configuration — (model-version × harness × tool surface × context-loader) — never to a brand.** "Claude" is a routing name; `model-id@config-hash` is what a track record can bind to. (This generalises "pin the runtime, not just the prompt" downward to the model layer, and is the promise-theory reading of [architecture axioms](../skills/microservices-architect/AXIOMS.md) 33–37: deterministic provenance, Runtime Trust Profile.)
