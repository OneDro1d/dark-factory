#!/usr/bin/env bash
# lib/redact.sh — pure text transforms shared by the handoff hooks.
# VR-4 (redact secrets/PII) and VR-3 (bound handoff size).
...
# that has no recognizable prefix and no adjacent keyword (e.g. a naked 40-char AWS
# secret access key sitting alone), because doing so reliably would over-redact
# commit SHAs and base64 blobs. Such secrets are caught only in their keyworded
# form (aws_secret_access_key=...). Hardened 2026-06-26 after an adversary gate.
