...
if [ "$KIND" = "template" ]; then
  ...
  say ""
  say "     To make it yours, once: open Claude Code in this directory and tell it"
  say "       \"Read START-HERE.md and execute it.\""
  say "     That runbook will:"
  say "       - make your own PRIVATE repo from this kit, keeping this one as kit-upstream (2B)"
  say "       - flip the marker to \"kind\": \"instance\" in your copy (2B)"
  say "       - give this machine its own record under instances/ (step 3)"
  say "       - declare the machine and install that record with --lock= (step 4)"
  say "     Customise your copy, never this shared repo: the next update here overwrites it."
  say ""
fi
...
if [ "$PLUG_N" -eq 0 ]; then
  ...
  while IFS= read -r p; do
    ...
    if command -v rsync >/dev/null 2>&1; then
      rsync -a --delete --exclude='.nfs*' "$PSRC_ABS/" "$PDEST_ABS/" \
